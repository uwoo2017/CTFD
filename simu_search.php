            ...
            <fieldset>
                ...
                if ($result = mysql_query($query)) {
                     ...
                     while ($row = mysql_fetch_array($result)) {
                         ...
                         print("<td class='even'>{$row['score1']}</td>");
                         print("<td>{$row['score2']}</td>");
                         print("<td class='even'>{$row['score3']}</td>");
                         print("<td>{$row['score4']}</td>");
                         print("<td>");
                         print('<form id="registForm" name="registForm" action="simu_revision.php" method="POST">');
                         print('<input type="hidden" name="key" value="'.$row['id'].'">');
                         print('<input type="submit" button class="square_btn" value="修正"></button><br><br>');
                         print('</form>');
                         print('<form name ="deleteForm" action="simu_delete.php" method="POST">');
                         print('<input type="hidden" name="key" value="'.$row['id'].'">');
