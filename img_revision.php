...
if (isset($_POST["update"])) {
    ...
    && !empty($_POST["text3"])&& !empty($_POST["text4"]) && isset($_POST["answer"]) ){
        ...
        try {
            ...
            $stmt = $pdo->prepare("UPDATE quiz SET id =?, question=?, text1 =?, text2 =?, text3=?, text4 =?, text5=?, answer=? WHERE id =?");

            $stmt->execute(array($id,$question, $text1, $text2, $text3, $text4, $text5,$answer,$b_id));  // パスワードのハッシュ化を行う（今回は文字列のみなのでbindValue(変数の内容が変わらない)を使用せず、直接excuteに渡しても問題ない）
            ...
            <fieldset>
                ...
                if(isset($_POST['id'])){
                    ...
                    while ($row = mysql_fetch_array($result)) {
                        ...
                        print('<td><textarea style="font-size:1.2em" name="text5" cols=15 rows=4>'.$row['text5'].'</textarea></td>');
                        print('<td><input type="radio" style="font-size:1.2em"  name="answer" value="1" '.$check1.'>1 ');
                        print('<input type="radio" style="font-size:1.2em"  name="answer" value="2" '.$check2.'>2 ');
                        print('<input type="radio" style="font-size:1.2em"  name="answer" value="3" '.$check3.'>3 ');
                        print('<input type="radio" style="font-size:1.2em"  name="answer" value="4" '.$check4.'>4');
                        print('<input type="radio" style="font-size:1.2em"  name="answer" value="5" '.$check5.'>5</td>');
                        echo "<td>";
